...
> [!IMPORTANT] 
> The purpose of this markdown scripts is to show how the DQCode scripts work together, and how to navigate the repository. However, it is not devised as a stand-alone executable. We thus ask the user to view this file as a guideline of how to use the scripts provided in the $\texttt{scripts/}$ folder in the Julia REPL. To this end, we outline the pipeline, and at every step indicate which commands to run in the REPL. For the general setup instructions, please refer to the README.md file. Having executed these instructions, one can proceed with this tutorial. The plots in this tutorial will also be displayed if you don't execute the commands, since we have provided some sample data in the `data` folder. But feel free to experiment with the pipeline, and gather your own!
