...
function network_setup(qec_code, register_sizes)
    ...
    mapping_transpositions = perm_to_transpositions(deepcopy(permutation))  
...
function code_setup(qec_code)
    ...
    if code_n(qec_code) < 20
        ...
        code_distance = 0
        @info "Code is too large to determine distance automatically"
    end
...
function run_circuit_search(exp_label::String, num_MCTS_runs::Int)
    ...
    if haskey(configs, exp_label)
        ...
        gottesman_encoding_circuit = encoding_gott(code_params, network_specs, cfg.folder)

        MCTS_gate_counts_per_run = Vector{Vector{Int64}}()
        ...
        genetic_search(code_params, network_specs, cfg.genetic_params, cfg.folder, warm_start = true, warm_start_gates = gottesman_encoding_circuit, label = "Baseline")
        df = DataFrame(GateCounts = [MCTS_gate_counts_per_run])
        CSV.write(joinpath(cfg.folder, "MCTS_optimal_gates_per_run.csv"), df)
...
function run_MCTS(exp_label::String)
    ...
    if haskey(configs, exp_label)
        ...
        network_specs = deserialize( joinpath(cfg.folder, "network_specs.jls"))
        code_params = deserialize( joinpath(cfg.folder, "code_params.jls"))
        monte_carlo_tree_search(code_params, network_specs, cfg.mcts_params, cfg.folder)
        return 42
    else
        error("This configuration label $exp_label was not found. Please add the respective data to the configuration file")
    end 
