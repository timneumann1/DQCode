module Circuit_Plots

using ..LogicalEnc
using QuantumClifford: @S_str
#using QECCore
using Quantikz: savecircuit, savetex, displaycircuit
...
function plot_gate_teleportation()
    circuit = []
    push!(circuit, HadamardGate)
    push!(circuit, VerifyOp(target_state , data_qubits)) 
    initial_state = S" IIIIIIZ IIIIIZI IIIIZII IIIZIII IIZIIII IZIIIII ZIIIIII"
    return mctrajectories(initial_state, circuit, trajectories=100)
end
